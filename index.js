...
//Após o fs ler o arquivo ele retorna o buffer(região fisica de memória do hardware) dos dados, para resolver isso usamos o JSON.parse para formatar em json
const itens = JSON.parse(itensdata);

const numItens = itens.map((el) =>
  sum.push(Number(el.quantity * el.value))
);
const plus = (x, y) => x + y;
